...
from Utils.utils_data import read_cea_target, read_cta_target, read_cpa_target,\
    write_semtab21_biodivtab_CEA_result, write_header_CTA_result, \
    write_CTA_result, write_CPA_result, load_cache_result
...
if __name__ == "__main__":
    ...
    result_dir = "{}_header".format(os.path.join(args.result_dir, log_fn))
    ...
    cea_targets = read_cea_target(os.path.join(args.data_dir,
                                               "BioDiv_CEA_WD_Round3_Targets.csv"))
    cta_targets = read_cta_target(os.path.join(args.data_dir,
                                               "BioDiv_CTA_WD_Round3_Targets.csv"))
    cpa_targets = read_cpa_target(os.path.join(args.data_dir,
                                               "BioDiv_CPA_WD_Round3_Targets.csv"))

    col_entities, col_types, col_properties = load_cache_result(args.log_fn)
    # write results
    write_semtab21_biodivtab_CEA_result(os.path.join(result_dir, 'CEA', "CEA.csv"),
                                        col_entities, cea_targets, wikidata_prefix="https://www.wikidata.org/wiki/")
    write_header_CTA_result(os.path.join(result_dir, 'CTA', "CTA.csv"),
                     col_entities, col_types, cta_targets, wikidata_prefix="https://www.wikidata.org/wiki/")
