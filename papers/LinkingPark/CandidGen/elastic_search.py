...
if __name__ == "__main__":
    ...
    parser.add_argument("--index_name", type=str, default="wikidata_rm_disambiguation")
    ...
    candid_dir = os.path.join(args.candid_dir, "Round{}".format(args.round))
    if not os.path.exists(os.path.join(candid_dir, "elastic_search")):
        os.mkdir(os.path.join(candid_dir, "elastic_search"))

    esutils = ElasticSearcher(index_name=args.index_name)

    esutils.generate_cands(os.path.join(candid_dir, "hash_matching", "unk_mentions.pkl"),
                           os.path.join(candid_dir, "elastic_search", "token_elasticsearch.pkl"),
                           top_k=args.top_k,
                           score_func='token')

    esutils.generate_cands(os.path.join(candid_dir, "hash_matching", "unk_mentions.pkl"),
                           os.path.join(candid_dir, "elastic_search", "token_char_elasticsearch.pkl"),
                           top_k=args.top_k,
                           score_func='token_char')